...
def word_search(word:str, text:str)->List[Tuple[int]]:
    ...
    """
    return [(ele.start(), ele.end()) for ele in re.finditer(word.lower(), text.lower())] if word is not None else []
...
def create_re_data(dataframe:pd.DataFrame,
                    ...
                    ent2:Text,
                    label:Text,
                    random_choice:bool=False)->pd.DataFrame:
    ...
    for index,row in tqdm(dataframe.iterrows(),total=dataframe.shape[0]):
        ...
        if len(res1) == 0 :
            ...
            if score1 < 1.0 and score1 > 0.6 : 
                low_score_sample.append({'Sentence':sentence, 'Query':b1, 'Result':e1, 'Score':score1})
        ...
        if len(res2) == 0 :
            ...
            if score2 < 1.0 and score2 > 6.0 : 
                low_score_sample.append({'Sentence':sentence, 'Query':b2, 'Result':e2, 'Score':score2})
        ...
        for j,r1 in enumerate(res1):
            ...
            for i,r in enumerate(res2):
                ...
                intersec = Intersection(list(range(r2[0],r2[1])), list(range(r1[0],r1[1])))
                if len(intersec) > 0 : 
                    continue
