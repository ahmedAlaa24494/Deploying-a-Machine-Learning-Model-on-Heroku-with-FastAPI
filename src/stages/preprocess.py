...
def preprocess(config_path: Text) -> None: 
    ...
    x_train, y_train, label_encoder, cat_features_encoder, num_features_encoder = process_data(X=train_df,
    ...
    x_test, y_test, label_encoder, cat_features_encoder, num_features_encoder = process_data(X=test_df,
        target=config['preprocess']['target'],
        do_train=False,
        cat_features=config['preprocess']['categorical_features'], 
        num_features=config['preprocess']['numerical_features'], 
    ...
    joblib.dump(label_encoder, f"{config['preprocess']['encoders_dir']}/label_encoder.gz")
    joblib.dump(cat_features_encoder, f"{config['preprocess']['encoders_dir']}/cat_features_encoder.gz")
    joblib.dump(num_features_encoder, f"{config['preprocess']['encoders_dir']}/num_features_encoder.gz")
    ## Save Features 
