...
def test_x_y_equalized(config):
    ...
    x, y, label_encoder, cat_features_encoder, num_features_encoder = process_data(X=train_df,
...
def test_fixed_encoders(clean_data,config): 
    base_label_encoder = joblib.load(f"{config['preprocess']['encoders_dir']}/label_encoder.gz")
    base_cat_encoder = joblib.load(f"{config['preprocess']['encoders_dir']}/cat_features_encoder.gz")
    base_num_encoder = joblib.load(f"{config['preprocess']['encoders_dir']}/num_features_encoder.gz")

    _, _, label_encoder, cat_features_encoder, num_features_encoder = process_data(X=clean_data,
        target=config['preprocess']['target'],
        do_train=True,
        cat_features=config['preprocess']['categorical_features'], 
        num_features=config['preprocess']['numerical_features'])    
    _, _, _, _, _ = process_data(X=clean_data,
        target=config['preprocess']['target'],
        do_train=False,
        cat_features=config['preprocess']['categorical_features'], 
        num_features=config['preprocess']['numerical_features'], 
...
def test_inference(config): 
    ...
    x_ex, _, _, _, _ = process_data(X=pd.DataFrame(example),
            target=None,
            do_train=False,
            cat_features=config['preprocess']['categorical_features'], 
            num_features=config['preprocess']['numerical_features'], 
